...
func _on_game_state_changed(new_state: GameState):
	match new_state:
		GameState.CHARACTER_SELECTION:
			...
			set_global_parameter_by_value('PointWinner', 0)
			set_global_parameter_by_value('Player1Score', 0)
			set_global_parameter_by_value('Player2Score', 0)
			set_global_parameter('RoundWon', 'No')
			set_global_parameter('PointWon', 'No')
		GameState.GAMEPLAY:
...
func _on_player_scored(player_index: int):
	...
	set_global_parameter_by_value('PointWinner', player_index + 1)
	set_global_parameter('PointWon', 'Yes')
